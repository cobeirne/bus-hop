...
{
    [TestClass()]
    public class SnappedApiClientControllerTests
    {
        [TestMethod()]
        public void BuildSnappedPointsQueryTest()
        {
            SnappedApiClientController controller = new SnappedApiClientController();
            ...
            string apiQuery = controller.BuildSnappedPointsQuery(routePoints.ToArray());

            Assert.IsTrue(apiQuery == "https://roads.googleapis.com/v1/snapToRoads?path=-38.145,144.355|-38.198,144.3|-38.157,144.356|-38.189,144.343&interpolate=true&key=AIzaSyAyEgv4_85K8azHU2fYz78xxGAT3ne3egU");
        }

        [TestMethod()]
        public void GetSnappedPointsTest()
        {
            SnappedApiClientController controller = new SnappedApiClientController();
            ...
            List<SnappedPoints> snappedPoints = controller.GetSnappedPoints(routePoints.ToArray());

            Assert.IsTrue(snappedPoints.Count() == 719);
        }
    }
}